...
import com.abc.live.util.ABCColorUtils;
...
public class ABCSwitchButton extends CompoundButton {
    ...
    public ABCSwitchButton(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        init(attrs);
    }

    public ABCSwitchButton(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs);
    }

    public ABCSwitchButton(Context context) {
        super(context);
        init(null);
    }
    ...
    private void init(AttributeSet attrs) {
        ...
        TypedArray ta = attrs == null ? null : getContext().obtainStyledAttributes(attrs, R.styleable.ABC_SwitchButton);
        if (ta != null) {
            thumbDrawable = ta.getDrawable(R.styleable.ABC_SwitchButton_kswThumbDrawable);
            thumbColor = ta.getColorStateList(R.styleable.ABC_SwitchButton_kswThumbColor);
            margin = ta.getDimension(R.styleable.ABC_SwitchButton_kswThumbMargin, margin);
            marginLeft = ta.getDimension(R.styleable.ABC_SwitchButton_kswThumbMarginLeft, margin);
            marginRight = ta.getDimension(R.styleable.ABC_SwitchButton_kswThumbMarginRight, margin);
            marginTop = ta.getDimension(R.styleable.ABC_SwitchButton_kswThumbMarginTop, margin);
            marginBottom = ta.getDimension(R.styleable.ABC_SwitchButton_kswThumbMarginBottom, margin);
            thumbWidth = ta.getDimension(R.styleable.ABC_SwitchButton_kswThumbWidth, thumbWidth);
            thumbHeight = ta.getDimension(R.styleable.ABC_SwitchButton_kswThumbHeight, thumbHeight);
            thumbRadius = ta.getDimension(R.styleable.ABC_SwitchButton_kswThumbRadius, Math.min(thumbWidth, thumbHeight) / 2.f);
            backRadius = ta.getDimension(R.styleable.ABC_SwitchButton_kswBackRadius, thumbRadius + density * 2f);
            backDrawable = ta.getDrawable(R.styleable.ABC_SwitchButton_kswBackDrawable);
            backColor = ta.getColorStateList(R.styleable.ABC_SwitchButton_kswBackColor);
            backMeasureRatio = ta.getFloat(R.styleable.ABC_SwitchButton_kswBackMeasureRatio, backMeasureRatio);
            animationDuration = ta.getInteger(R.styleable.ABC_SwitchButton_kswAnimationDuration, animationDuration);
            fadeBack = ta.getBoolean(R.styleable.ABC_SwitchButton_kswFadeBack, true);
            tintColor = ta.getColor(R.styleable.ABC_SwitchButton_kswTintColor, tintColor);
            textOn = ta.getString(R.styleable.ABC_SwitchButton_kswTextOn);
            textOff = ta.getString(R.styleable.ABC_SwitchButton_kswTextOff);
            textMarginH = Math.max(textMarginH, backRadius / 2);
            textMarginH = ta.getDimension(R.styleable.ABC_SwitchButton_kswTextMarginH, textMarginH);
            autoAdjustTextPosition = ta.getBoolean(R.styleable.ABC_SwitchButton_kswAutoAdjustTextPosition, autoAdjustTextPosition);
            ta.recycle();
        }
        ...
        if (!mIsThumbUseDrawable && mThumbColor == null) {
            mThumbColor = ABCColorUtils.generateThumbColorWithTintColor(mTintColor);
        ...
        if (!mIsBackUseDrawable && mBackColor == null) {
            mBackColor = ABCColorUtils.generateBackColorWithTintColor(mTintColor);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        // action the view going to change to when finger released
        boolean nextStatus;
    ...
    /**
     * return the action based on position of thumb
     *
     * @return
     */
    ...
    public void setChecked(final boolean checked) {
        // animate before super.setChecked() become userId may call setChecked again in OnCheckedChangedListener
