...
import com.abc.live.pdf.listener.OnDrawListener;
import com.abc.live.pdf.listener.OnErrorListener;
import com.abc.live.pdf.listener.OnLoadCompleteListener;
import com.abc.live.pdf.listener.OnPageChangeListener;
import com.abc.live.pdf.listener.OnPageScrollListener;
import com.abc.live.pdf.listener.OnRenderListener;
import com.abc.live.pdf.model.PagePart;
import com.abc.live.pdf.scroll.ScrollHandle;
import com.abc.live.pdf.source.AssetSource;
import com.abc.live.pdf.source.DocumentSource;
import com.abc.live.pdf.source.FileSource;
import com.abc.live.pdf.source.UriSource;
import com.abc.live.pdf.util.ArrayUtils;
import com.abc.live.pdf.util.Constants;
import com.abc.live.pdf.util.MathUtils;
import com.abc.live.pdf.util.Util;
 ...
 * - RecordUserPage = A page as defined by the userId.
 * By default, they're the same. But the userId can change the pages order
...
public class PDFView extends RelativeLayout {
    ...
    /**
     * Rendered parts go to the cache mLivingRoomManager
     */
    CacheManager cacheManager;

    /**
     * Animation mLivingRoomManager manage all offset and zoom animation
     */
    private AnimationManager animationManager;

    /**
     * Drag mLivingRoomManager manage all touch events
     */
    private DragPinchManager dragPinchManager;

    /**
     * The pages the userId want to display in order
     * (ex: 0, 2, 2, 8, 8, 1, 1, 1)
     */
     ...
     * Can not be forced on older API versions (< Build.VERSION_CODES.KITKAT) as the MyGestureDetectors does
    ...
    void loadError(Throwable t) {
        ...
        if (this.onErrorListener != null) {
            ...
            ALog.e("PDFView", "load pdf error", t);
        }
    }
    ...
    /**
     * Given the RecordUserPage number, this method restrict it
     * to be sure it's an existing page. It takes care of
     * using the userId defined pages if any.
     *
    ...
    public void fitToWidth(int page) {
        if (state != State.SHOWN) {
            ALog.e(TAG, "Cannot fit, document not rendered yet");
            return;
        }
        fitToWidth();
        jumpTo(page);
